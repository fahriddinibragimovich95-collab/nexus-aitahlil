/**
 * NeXuS AI — Netlify Serverless Function (PRODUCTION READY)
...
// Yahoo Finance survey/notice xabarlarini o'chirish
try {
...
exports.handler = async function (event, context) {
  ...
  try {
    ...
    // ⚡ TICKERNI TO'LIQ TOZALASH: #AAPL, $MULN, +MSFT, *NVDA* → AAPL, MULN, MSFT, NVDA
    ...
    if (ticker.length < 1 || ticker.length > 6) {
      return {
        ...
        headers,
        body: JSON.stringify({
          error: `Noto'g'ri ticker formati: "${ticker}". US Stocks tikerlari 1-6 harfdan iborat bo'lishi kerak.`,
        }),
      };
    }
  } catch (e) {
  ...
  try {
    ...
    try {
      ...
      if (!yfRaw || !yfRaw.price) {
        throw new Error("Yahoo Finance bo'sh javob qaytardi");
      }
    } catch (e) {
      console.error("Yahoo Finance xato:", e.message);
      return {
        statusCode: 404,
        headers,
        body: JSON.stringify({
          error: `"${ticker}" uchun bozor ma'lumotlari topilmadi. Ticker to'g'riligini tekshiring (faqat US Stocks: AAPL, MSFT, NVDA va h.k.).`,
          detail: e.message,
        }),
      };
    }
    ...
    if (!data.price || data.price <= 0) {
      return {
        ...
        headers,
        body: JSON.stringify({
          error: `"${ticker}" uchun aniq narx ma'lumotlari topilmadi. Bu ticker faol savdoda emas yoki noto'g'ri kiritilgan.`,
        }),
      };
    }
...
function extractYFData(yf) {
  ...
  // yahoo-finance2 ba'zan object {raw, fmt} qaytaradi, ba'zan oddiy raqam
  const num = (v) => {
...
function calculateValuations(d) {
  ...
  const allVals = [dcf, peHist, peMarket, evEbitda, ps, fcfYield, epv, graham, ruleOf40, peerAvg];
...
function formatFairValueResult(ticker, d, v, raw) {
  ...
  const valuations = [
    { name: "DCF (Chegirmalangan Pul Oqimi)", shortName: "DCF", value: v.dcf, icon: "💎", desc: "5 yillik FCF prognozi, WACC=9.5%, terminal o'sish=2.5%" },
    { name: "P/E (Tarixiy Multiplier)", shortName: "P/E Tarixiy", value: v.peHist, icon: "📐", desc: `EPS × Tarixiy P/E (${d.historicalPE?.toFixed(1)}x)` },
    { name: "P/E (Konservativ Bozor)", shortName: "P/E Konservativ", value: v.peMarket, icon: "📏", desc: `EPS × min(P/E, 18) — konservativ baholash` },
    { name: "EV/EBITDA", shortName: "EV/EBITDA", value: v.evEbitda, icon: "🏗️", desc: "EBITDA × 11x sektoral multiplier + Net Kash" },
    { name: "Price/Sales", shortName: "P/S", value: v.ps, icon: "💼", desc: `Revenue × P/S multiplier (${d.historicalPS?.toFixed(1)}x)` },
    { name: "FCF Yield Modeli", shortName: "FCF Yield", value: v.fcfYield, icon: "💧", desc: "FCF / 0.08 — maqsadli 8% FCF yield" },
    { name: "EPV (Daromad Quvvati Qiymati)", shortName: "EPV", value: v.epv, icon: "⚡", desc: "EPS / WACC — o'sishsiz intrinsik qiymat" },
    { name: "Graham Raqami", shortName: "Graham", value: v.graham, icon: "🛡️", desc: "√(22.5 × EPS × Kitob Qiymati)" },
    { name: "Rule of 40", shortName: "Rule of 40", value: v.ruleOf40, icon: "🚦", desc: `Ball: ${v.ruleScore?.toFixed(1)} (O'sish% + FCF Margin%)` },
    { name: "Sektor Tengdoshlari", shortName: "Peer Avg", value: v.peerAvg, icon: "🤝", desc: `Revenue × Sektoral tengdosh multiplier` },
  ];
...
function runPennyFilters(ticker, data) {
  ...
  if (floatEst > 0 && floatEst < 30) {
    filters.push({ id: 1, name: "Float", criteria: "< 30M ta aksiya", status: "pass", value: `~${floatEst.toFixed(1)}M ta`, comment: "Muvaffaqiyatli zona — past float yuqori volatillikka olib keladi" });
    passCount++;
  } else if (floatEst >= 30 && floatEst < 60) {
    filters.push({ id: 1, name: "Float", criteria: "< 30M ta aksiya", status: "warn", value: `~${floatEst.toFixed(1)}M ta`, comment: "Chegarada — harakatchanlik pasayishi mumkin" });
    warnCount++;
  } else {
    filters.push({ id: 1, name: "Float", criteria: "< 30M ta aksiya", status: "fail", value: floatEst > 0 ? `~${floatEst.toFixed(1)}M ta` : "Ma'lumot yo'q", comment: "Float juda katta — momentum pop ehtimoli past" });
    failCount++;
  }

  // FILTER 2: MARKET CAP
  if (mc > 0 && mc < 20) {
    filters.push({ id: 2, name: "Market Cap", criteria: "< $20M", status: "pass", value: `$${mc.toFixed(2)}M`, comment: "Micro-cap zona — yuqori salohiyatli harakat imkoniyati" });
    passCount++;
  } else if (mc >= 20 && mc < 60) {
    filters.push({ id: 2, name: "Market Cap", criteria: "< $20M", status: "warn", value: `$${mc.toFixed(2)}M`, comment: "Small-cap chegarasida — ehtiyotkor bo'ling" });
    warnCount++;
  } else {
    filters.push({ id: 2, name: "Market Cap", criteria: "< $20M", status: "fail", value: mc > 0 ? `$${mc.toFixed(2)}M` : "Ma'lumot yo'q", comment: "Bozor qiymati juda katta — penny dinamikasi yo'q" });
    failCount++;
  }

  // FILTER 3: NARX
  if (price > 0.01 && price <= 5) {
    filters.push({ id: 3, name: "Narx Oralig'i", criteria: "$0.01 — $5", status: "pass", value: `$${price.toFixed(4)}`, comment: "Klassik penny stock narxi oralig'ida" });
    passCount++;
  } else if (price > 5 && price <= 10) {
    filters.push({ id: 3, name: "Narx Oralig'i", criteria: "$0.01 — $5", status: "warn", value: `$${price.toFixed(2)}`, comment: "Chegarada — ba'zi setuplarda ishlaydi" });
    warnCount++;
  } else {
    filters.push({ id: 3, name: "Narx Oralig'i", criteria: "$0.01 — $5", status: "fail", value: price > 0 ? `$${price.toFixed(2)}` : "Ma'lumot yo'q", comment: price <= 0.01 ? "Narx juda past — delisting xavfi" : "Narx penny chegarasidan yuqori" });
    failCount++;
  }

  // FILTER 4: REVERSE SPLIT
  if (data.high_52 > 0) {
    const drop = ((price - data.high_52) / data.high_52) * 100;
    if (drop < -80) {
      filters.push({ id: 4, name: "Reverse Split Xavfi", criteria: "52w high dan < 80% tushmasin", status: "fail", value: `${drop.toFixed(1)}% tushgan`, comment: "Reverse Split ehtimoli YUQORI — katta xavf" });
      failCount++;
    } else if (drop < -50) {
      filters.push({ id: 4, name: "Reverse Split Xavfi", criteria: "52w high dan < 80% tushmasin", status: "warn", value: `${drop.toFixed(1)}% tushgan`, comment: "O'rta xavf — kuzatishda bo'ling" });
      warnCount++;
    } else {
      filters.push({ id: 4, name: "Reverse Split Xavfi", criteria: "52w high dan < 80% tushmasin", status: "pass", value: `${drop.toFixed(1)}% tushgan`, comment: "RS xavfi past — qabul qilinadi" });
      passCount++;
    }
  } else {
    filters.push({ id: 4, name: "Reverse Split Xavfi", criteria: "52w high dan < 80% tushmasin", status: "warn", value: "Ma'lumot yo'q", comment: "52w high topilmadi — qo'lda tekshiring" });
    warnCount++;
  }
  ...
  if (vol >= 500000) {
    filters.push({ id: 5, name: "Savdo Hajmi", criteria: "> 500K lot", status: "pass", value: formatVolume(vol), comment: "Yetarli likvidlik — kirish/chiqish oson" });
    passCount++;
  } else if (vol >= 100000) {
    filters.push({ id: 5, name: "Savdo Hajmi", criteria: "> 500K lot", status: "warn", value: formatVolume(vol), comment: "O'rta likvidlik — slippage xavfi bor" });
    warnCount++;
  } else {
    filters.push({ id: 5, name: "Savdo Hajmi", criteria: "> 500K lot", status: "fail", value: vol > 0 ? formatVolume(vol) : "Ma'lumot yo'q", comment: "Past likvidlik — manipulyatsiya xavfi YUQORI" });
    failCount++;
  }
  ...
  if (rvol >= 2) {
    filters.push({ id: 6, name: "Nisbiy Hajm (RVOL)", criteria: "> 2x o'rtacha", status: "pass", value: `${rvol.toFixed(2)}x`, comment: "Yuqori RVOL — momentum harakatda" });
    passCount++;
  } else if (rvol >= 1) {
    filters.push({ id: 6, name: "Nisbiy Hajm (RVOL)", criteria: "> 2x o'rtacha", status: "warn", value: rvol > 0 ? `${rvol.toFixed(2)}x` : "Ma'lumot yo'q", comment: "O'rtacha RVOL — trigger kuting" });
    warnCount++;
  } else {
    filters.push({ id: 6, name: "Nisbiy Hajm (RVOL)", criteria: "> 2x o'rtacha", status: "fail", value: rvol > 0 ? `${rvol.toFixed(2)}x` : "Ma'lumot yo'q", comment: "Past RVOL — harakat yo'q" });
    failCount++;
  }
  ...
  return {
    filters, passCount, failCount, warnCount,
...
function calculateGrowthScore(data) {
  ...
  if (de < 50) { score += 15; factors.push({ name: "D/E Nisbati", value: `${de.toFixed(0)}%`, points: 15, status: "excellent" }); }
  else if (de < 150) { score += 8; factors.push({ name: "D/E Nisbati", value: `${de.toFixed(0)}%`, points: 8, status: "good" }); }
  else { factors.push({ name: "D/E Nisbati", value: `${de.toFixed(0)}%`, points: 0, status: "poor" }); }
...
function calculateValueScore(data) {
  ...
  if (pe > 0 && pe < 15) { score += 25; factors.push({ name: "P/E Nisbati", value: `${pe.toFixed(1)}x`, points: 25, status: "excellent" }); }
  else if (pe > 0 && pe < 25) { score += 12; factors.push({ name: "P/E Nisbati", value: `${pe.toFixed(1)}x`, points: 12, status: "good" }); }
  else if (pe > 0 && pe < 40) { score += 5; factors.push({ name: "P/E Nisbati", value: `${pe.toFixed(1)}x`, points: 5, status: "fair" }); }
  else { factors.push({ name: "P/E Nisbati", value: pe > 0 ? `${pe.toFixed(1)}x` : "N/A", points: 0, status: "poor" }); }

  const pb = data.priceToBook || 0;
  if (pb > 0 && pb < 1.5) { score += 20; factors.push({ name: "P/B Nisbati", value: `${pb.toFixed(2)}x`, points: 20, status: "excellent" }); }
  else if (pb > 0 && pb < 3) { score += 10; factors.push({ name: "P/B Nisbati", value: `${pb.toFixed(2)}x`, points: 10, status: "good" }); }
  else { factors.push({ name: "P/B Nisbati", value: pb > 0 ? `${pb.toFixed(2)}x` : "N/A", points: 0, status: "poor" }); }

  const margin = data.high_52 > 0
    ? ((data.high_52 - data.price) / data.high_52) * 100
    : 0;
  if (margin > 30) { score += 20; factors.push({ name: "Xavfsizlik Marjasi", value: `${margin.toFixed(1)}% past`, points: 20, status: "excellent" }); }
  else if (margin > 15) { score += 10; factors.push({ name: "Xavfsizlik Marjasi", value: `${margin.toFixed(1)}% past`, points: 10, status: "good" }); }
  else { factors.push({ name: "Xavfsizlik Marjasi", value: margin > 0 ? `${margin.toFixed(1)}% past` : "N/A", points: 0, status: "poor" }); }
...
// ─────────────────────────────────────────
// AI PROMPT QURISH (Swing Trading Tahlili)
// ─────────────────────────────────────────
...
Debt/Equity (D/E): ${data.debt_equity?.toFixed(1) || "N/A"} | Current Ratio: ${data.current_ratio?.toFixed(2) || "N/A"}
...
Quyidagi tartibda CHUQUR Swing Trading tahlili tayyorla:

1. **Kompaniya haqida**: Nima qiladi, qaysi sektor?
2. **Moliyaviy holat**: Barqarorlik, manipulyatsiya xavfi, kash pozitsiyasi.
3. **Texnik darajalar**:
   - 🟢 Qo'llab-quvvatlash (Support): aniq narxlar
   - 🔴 Qarshilik (Resistance): aniq narxlar
4. **Risklarni boshqarish**:
   - Stop Loss: aniq narx
   - Position size: kapitalning necha %
5. **Target narxlar (Swing 1-4 hafta)**:
   - 🎯 Target 1
   - 🎯 Target 2
...
Professional trader tilida yoz.`;
  ...
  if (mode === "growth") {
    return `Sen Wall Street'ning yetakchi Growth Stock va Swing Trading tahlilchisisan. FAQAT O'ZBEK TILIDA professional javob ber.

${baseStats}

3-6 oylik Swing Trade uchun CHUQUR tahlil tayyorla:

1. **O'sish dinamikasi**: Revenue, EPS o'sish sur'ati va sifati.
2. **Moliyaviy mustahkamlik**: FCF, margin tendensiyasi, kash.
3. **Bozor pozitsiyasi**: Sektor lideri yoki lagerda?
4. **Texnik darajalar**:
   - 🟢 Qo'llab-quvvatlash (Support): aniq narxlar
   - 🔴 Qarshilik (Resistance): aniq narxlar
5. **Risklarni boshqarish**:
   - Stop Loss: aniq narx
   - Risk/Reward nisbati: 1:?
6. **Target narxlar**:
   - 🎯 Qisqa muddatli (1-2 oy)
   - 🎯 O'rta muddatli (3-6 oy)
7. **3 ta asosiy XAVF omillari**
8. **Yakuniy HUKM**: ✅ SOTIB OL / ⚠️ KUT / ❌ SOTMA — aniq sabab bilan.

Professional va aniq yoz.`;
  }

  if (mode === "value") {
    return `Sen Warren Buffett va Benjamin Graham uslubidagi Value Investing + Swing Trading tahlilchisisan. FAQAT O'ZBEK TILIDA professional javob ber.

${baseStats}

3-6 oylik Swing Trade uchun CHUQUR Value tahlili tayyorla:

1. **Intrinsik qiymat**: P/E, P/B, P/S nisbatlar bozor o'rtachasiga nisbatan arzonmi?
2. **Moliyaviy mustahkamlik**: D/E, likvidlik, pul oqimi sifati.
3. **Xavfsizlik marjasi (Margin of Safety)**: 52w highdan necha %?
4. **Raqobat ustunligi (Moat)**: Asosiy afzallik nima?
5. **Texnik darajalar**:
   - 🟢 Qo'llab-quvvatlash (Support): aniq narxlar
   - 🔴 Qarshilik (Resistance): aniq narxlar
6. **Risklarni boshqarish**:
   - Stop Loss: aniq narx
   - Position size tavsiyasi
7. **Target narxlar**:
   - 🎯 Adolatli qiymat (Fair Value)
   - 🎯 Optimistik target
8. **Yakuniy HUKM**: ✅ SOTIB OL / ⚠️ KUT / ❌ SOTMA — asosiy sabab va maqsadli narx.

Buffett-Graham tamoyillariga asoslanib yoz.`;
  }

  return `${baseStats}\n\nProfessional o'zbek tilida chuqur Swing Trading tahlilini tayyorla. Support/Resistance, Stop Loss, Target narxlar va yakuniy Hukm kiritilsin.`;
}
